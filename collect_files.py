...
def organize_nifti_files(main_folder):
    ...
    for patient_folder in main_path.iterdir():
        if patient_folder.is_dir() and patient_folder.name not in ["Images", "Labels"]:
            ...
            for file in patient_folder.glob('*.nii*'):  # Matches .nii and .nii.gz
                ...
                if 'dwi' in filename:
                    dest = images_folder / f"{patient_folder.name}_{file.name}"
    ...
    print(f"Copied {dwi_count} DWI images to {images_folder}")
    print(f"Copied {mask_count} masks to {labels_folder}")

# Usage
if __name__=="__main__":

    dataset_path = "D:\\Trabajo\\Code\\Brain Stroke Segmentation\\ISLE2022"
